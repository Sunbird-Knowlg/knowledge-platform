...
object NodeUtil {
    ...
    def serialize(node: Node, fields: util.List[String]): util.Map[String, AnyRef] = {
        val metadataMap = node.getMetadata
        metadataMap.put("identifier", node.getIdentifier)
        if (CollectionUtils.isNotEmpty(fields))
            metadataMap.keySet.retainAll(fields)
        val jsonProps = DefinitionNode.fetchJsonProps(node.getGraphId, "1.0", node.getObjectType)
        ...
        val definitionMap = DefinitionNode.getRelationDefinitionMap(node.getGraphId, "1.0", node.getObjectType).asJava
