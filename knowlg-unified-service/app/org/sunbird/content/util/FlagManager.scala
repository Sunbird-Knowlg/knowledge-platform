...
object FlagManager {
  ...
  def updateCollection(request: Request)(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[Node] = {
    fetchHierarchy(request).map(hierarchyString => {
      if (!hierarchyString.asInstanceOf[String].isEmpty) {
        ...
        request.put(ContentHierarchyConstants.HIERARCHY, JsonUtils.serialize(hierarchyMap))
      }
  ...
  private def fetchHierarchy(request: Request)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[Any] = {
    oec.graphService.readExternalProps(request, List(ContentHierarchyConstants.HIERARCHY)).map(resp => {
      resp.getResult.asScala.toMap.getOrElse(ContentHierarchyConstants.HIERARCHY, "").asInstanceOf[String]
    }) recover { case e: ResourceNotFoundException => TelemetryManager.log("No hierarchy is present in cassandra for identifier:" + request.get(ContentHierarchyConstants.IDENTIFIER)) }
  }
  ...
  def updateContentFlag(node: Node, request: Request)(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[Node] ={
    ...
    if(StringUtils.equalsIgnoreCase(mimeType, ContentHierarchyConstants.COLLECTION_MIME_TYPE)){
