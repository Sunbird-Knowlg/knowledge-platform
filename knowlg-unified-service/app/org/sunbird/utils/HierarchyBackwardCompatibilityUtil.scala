...
object HierarchyBackwardCompatibilityUtil {
    ...
    def setNewObjectType(node: Node) = {
        ...
        if (StringUtils.isNotBlank(mimeType) && StringUtils.equalsIgnoreCase(mimeType, ContentHierarchyConstants.COLLECTION_MIME_TYPE)) {
            metadata.put(ContentHierarchyConstants.OBJECT_TYPE, ContentHierarchyConstants.COLLECTION_OBJECT_TYPE)
            node.setObjectType(ContentHierarchyConstants.COLLECTION_OBJECT_TYPE)
        } else if ((StringUtils.isNotBlank(contentType) && StringUtils.equalsIgnoreCase(contentType, ContentHierarchyConstants.ASSET_CONTENT_TYPE))
            || (StringUtils.isNotBlank(primaryCategory) && StringUtils.equalsIgnoreCase(primaryCategory, ContentHierarchyConstants.ASSET_CONTENT_TYPE))) {
            metadata.put(ContentHierarchyConstants.OBJECT_TYPE, ContentHierarchyConstants.ASSET_OBJECT_TYPE)
            node.setObjectType(ContentHierarchyConstants.ASSET_OBJECT_TYPE)
        } else {
            metadata.put(ContentHierarchyConstants.OBJECT_TYPE, objectType)
        }
    }
}