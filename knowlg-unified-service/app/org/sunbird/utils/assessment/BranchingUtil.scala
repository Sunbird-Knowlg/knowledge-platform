...
object BranchingUtil {
	...
	def generateBranchingRecord(nodesModified: util.HashMap[String, AnyRef]): util.HashMap[String, AnyRef] = {
		...
		idSet.map(id => {
			val nodeMetaData = nodesModified.getOrDefault(id, new util.HashMap()).asInstanceOf[util.Map[String, AnyRef]].getOrDefault(Constants.METADATA, new util.HashMap()).asInstanceOf[util.Map[String, AnyRef]]
			val containsBL = nodeMetaData.containsKey(Constants.BRANCHING_LOGIC)
			branchingRecord.put(id, new util.HashMap[String, AnyRef]() {
				{
					if (containsBL) {
						put(Constants.BRANCHING_LOGIC, nodeMetaData.get(Constants.BRANCHING_LOGIC))
						nodeMetaData.remove(Constants.BRANCHING_LOGIC)
					}
					put(Constants.CONTAINS_BL, containsBL.asInstanceOf[AnyRef])
					put(Constants.COPY_OF, nodeMetaData.get(Constants.COPY_OF).asInstanceOf[String])
				}
			})
			nodeMetaData.remove(Constants.COPY_OF)
		})
		branchingRecord
	}
	...
	def hierarchyRequestModifier(request: Request, branchingRecord: util.HashMap[String, AnyRef], identifiers: util.Map[String, String]) = {
		...
		branchingRecord.keySet().asScala.toList.map(id => {
			...
			val nodeMetaData = node.get(Constants.METADATA).asInstanceOf[util.HashMap[String, AnyRef]]
			val newId = identifiers.get(id)
			if (nodeInfo.get(Constants.CONTAINS_BL).asInstanceOf[Boolean]) {
				val branchingLogic = nodeInfo.get(Constants.BRANCHING_LOGIC).asInstanceOf[util.HashMap[String, AnyRef]]
				branchingLogicModifier(branchingLogic, oldToNewIdMap)
				nodeMetaData.put(Constants.BRANCHING_LOGIC, branchingLogic)
			}
			node.put(Constants.IS_NEW, false.asInstanceOf[AnyRef])
		...
		hierarchy.keySet().asScala.toList.map(id => {
			...
			val children = nodeHierarchy.get(Constants.CHILDREN).asInstanceOf[util.ArrayList[String]]
			...
			nodeHierarchy.put(Constants.CHILDREN, newChildrenList)
	...
	def branchingLogicModifier(branchingLogic: util.HashMap[String, AnyRef], oldToNewIdMap: util.Map[String, String]): Unit = {
		branchingLogic.keySet().asScala.toList.map(identifier => {
			...
			nodeBL.keySet().asScala.toList.map(key => {
				if (StringUtils.equalsIgnoreCase(key, Constants.TARGET)) branchingLogicArrayHandler(nodeBL, Constants.TARGET, oldToNewIdMap)
				else if (StringUtils.equalsIgnoreCase(key, Constants.PRE_CONDITION)) preConditionHandler(nodeBL, oldToNewIdMap)
				else if (StringUtils.equalsIgnoreCase(key, Constants.SOURCE)) branchingLogicArrayHandler(nodeBL, Constants.SOURCE, oldToNewIdMap)
			})
	...
	def getIdentifierMapping(branchingRecord: util.HashMap[String, AnyRef], identifiers: util.Map[String, String]): util.Map[String, String] = {
		...
		branchingRecord.keySet().asScala.toList.map(id => {
			val nodeInfo = branchingRecord.get(id).asInstanceOf[util.HashMap[String, AnyRef]]
			val newId = identifiers.get(id)
			val oldId = nodeInfo.get(Constants.COPY_OF).asInstanceOf[String]
	...
	def preConditionHandler(nodeBL: util.HashMap[String, AnyRef], oldToNewIdMap: util.Map[String, String]): Unit = {
		val preCondition = nodeBL.get(Constants.PRE_CONDITION).asInstanceOf[util.HashMap[String, AnyRef]]
		preCondition.keySet().asScala.toList.map(key => {
			...
			condition.keySet().asScala.toList.map(logicOp => {
				...
				val preConditionVar = sourceQuestionRecord.get(Constants.PRE_CONDITION_VAR).asInstanceOf[String]
				...
				if (oldToNewIdMap.containsKey(stringArray(0))) {
					...
					sourceQuestionRecord.put(Constants.PRE_CONDITION_VAR, newString)
				}
			})
		})
	}
