	...
	def review(request: Request): Future[Response] = {
		...
		DataNode.read(readReq).map(node => {
			if (CollectionUtils.isNotEmpty(node.getOutRelations)) {
				...
				val itemRels: List[Relation] = node.getOutRelations.asScala.filter((rel: Relation) => StringUtils.equalsAnyIgnoreCase("AssessmentItem", rel.getEndNodeObjectType)).filterNot((reln: Relation) => StringUtils.equalsAnyIgnoreCase("Retired", reln.getEndNodeMetadata.get("status").toString)).toList
				...
				if (CollectionUtils.isNotEmpty(draftRelIds.asJava)) {
				...
				val newRels: util.List[util.HashMap[String, AnyRef]] = itemRels.sortBy((rel: Relation) => rel.getMetadata.get("IL_SEQUENCE_INDEX").asInstanceOf[Long])(Ordering.Long).map(rel => {
					new util.HashMap[String, AnyRef]() {{put("identifier", rel.getEndNodeId);}}}).asJava
