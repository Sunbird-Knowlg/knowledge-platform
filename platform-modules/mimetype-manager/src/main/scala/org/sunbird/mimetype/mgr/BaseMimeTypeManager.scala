...
class BaseMimeTypeManager(implicit ss: StorageService) {
	...
	def uploadArtifactToCloud(uploadedFile: File, identifier: String, filePath: String): Array[String] = {
		var urlArray = new Array[String](2)
		try {
			val folder = if(StringUtils.isNotBlank(filePath)) Platform.getString(CONTENT_FOLDER, "content") + File.separator + filePath + File.separator + Slug.makeSlug(identifier, true) + File.separator + Platform.getString(ARTIFACT_FOLDER, "artifact") else Platform.getString(CONTENT_FOLDER, "content") + File.separator + Slug.makeSlug(identifier, true) + File.separator + Platform.getString(ARTIFACT_FOLDER, "artifact")
