...
class AssetMimeTypeMgrImplTest extends AsyncFlatSpec with Matchers with AsyncMockFactory {
  ...
  "upload with valid file" should "return artifactUrl with successful response" in {
    ...
    val resFuture = new AssetMimeTypeMgrImpl().upload(identifier, node, new File(Resources.getResource("filesToZip/human_vs_robot-.jpg").toURI), null)
  ...
  "upload with file" should "throw client exception" in {
    val exception = intercept[ClientException] {
      new AssetMimeTypeMgrImpl().upload("do_123", new Node(), new File("/tmp/test.pdf"), null)
    }
  ...
  "upload with valid fileUrl" should "return artifactUrl with successful response" in {
    ...
    val resFuture = new AssetMimeTypeMgrImpl().upload("do_123", node, inputUrl, null)
    resFuture.map(result => {
      assert(null != result)
