...
class ApkMimeTypeMgrImplTest extends AsyncFlatSpec with Matchers with AsyncMockFactory  {
    ...
    it should "upload apk file and return public url" in {
        ...
        val resFuture = new ApkMimeTypeMgrImpl().upload(identifier, node, new File(Resources.getResource("uploadAPK.apk").toURI), None, false)
    ...
    it should "upload apk fileurl and return public url" in {
        ...
        val identifier = "do_123"
        implicit val ss = mock[StorageService]
        val resFuture = new ApkMimeTypeMgrImpl().upload(identifier, node, "https://ekstep-public-prod.s3-ap-south-1.amazonaws.com/content/do_30083930/artifact/aser-6.0.0.17_215_1505458979_1505459188679.apk", None, false)
        resFuture.map(result => {
            println("Response: " + result)
            result
        })
