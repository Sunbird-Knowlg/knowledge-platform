...
class DefaultMimeTypeImplTest extends AsyncFlatSpec with Matchers with AsyncMockFactory {
    ...
    "upload pdf file" should "upload pdf file and return public url" in {
        ...
        val resFuture = new DefaultMimeTypeMgrImpl().upload(identifier, node, new File(Resources.getResource("sample.pdf").toURI), None)
    ...
    it should "upload apk fileurl and return public url" in {
        ...
        val identifier = "do_123"
        implicit val ss = mock[StorageService]
        val resFuture = new DefaultMimeTypeMgrImpl().upload(identifier, node, "https://ekstep-public-prod.s3-ap-south-1.amazonaws.com/content/do_30083930/artifact/aser-6.0.0.17_215_1505458979_1505459188679.apk", None)
        resFuture.map(result => {
            println("Response: " + result)
            result
        })
    ...
    "upload pdf file" should "throw client error for mimeType mismatch" in {
        ...
        val exception = intercept[ClientException] {
            new DefaultMimeTypeMgrImpl().upload("do_123", node, new File(Resources.getResource("sample.pdf").toURI), None)
        }
