...
class HtmlMimeTypeMgrImplTest extends AsyncFlatSpec with Matchers with AsyncMockFactory {
    ...
    "upload invalid html file" should "return client exception" in {
        val exception = intercept[ClientException] {
            new HtmlMimeTypeMgrImpl().upload("do_123", new Node(), new File(Resources.getResource("invalidHtmlContent.zip").toURI), None, false)
        }
    ...
    "upload invalid html file url" should "return client exception" in {
        val exception = intercept[ClientException] {
            new HtmlMimeTypeMgrImpl().upload("do_123", new Node(), "invalid.html", None, false)
        }
    ...
    it should "upload HTML zip file and return public url" in {
        ...
        val resFuture = new HtmlMimeTypeMgrImpl().upload(identifier, node, new File(Resources.getResource("validHtml.zip").toURI), None, false)
    ...
    it should "upload HTML zip Url and return public url" in {
        ...
        (ss.uploadFile(_: String, _: File, _: Option[Boolean])).expects(*, *, *).returns(Array(identifier, identifier))
        (ss.uploadDirectory(_:String, _:File, _: Option[Boolean])).expects(*, *, *)
        val resFuture = new HtmlMimeTypeMgrImpl().upload(identifier, node, "https://sunbirddev.blob.core.windows.net/sunbird-content-dev/content/do_1126981080516608001180/artifact/1.-_1550062024041.zip", None, false)
        resFuture.map(result => {
            assert(null != result)
            assert(result.nonEmpty)
            assert("do_1234" == result.getOrElse("identifier", ""))
