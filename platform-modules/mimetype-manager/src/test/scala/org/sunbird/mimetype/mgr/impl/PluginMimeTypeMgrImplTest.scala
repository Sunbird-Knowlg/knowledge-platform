...
class PluginMimeTypeMgrImplTest extends AsyncFlatSpec with Matchers with AsyncMockFactory {
    ...
    it should "upload plugin zip file and return public url" in {
        ...
        val resFuture = new PluginMimeTypeMgrImpl().upload(identifier, node, new File(Resources.getResource("plugin.zip").toURI), null)
    ...
    it should "upload plugin zip file url and return public url" in {
        ...
        (ss.uploadFile(_: String, _: File, _: Option[Boolean])).expects(*, *, *).returns(Array(identifier, identifier))
        (ss.uploadDirectory(_: String, _: File, _: Option[Boolean])).expects(*, *, *)
        val resFuture = new PluginMimeTypeMgrImpl().upload(identifier, node, "https://sunbirddev.blob.core.windows.net/sunbird-content-dev/content/org.ekstep.summary/artifact/org.ekstep.summary-1.0_1576230748183.zip", null)
        resFuture.map(result => {
            println("Response: " + result)
            result
        })
    ...
    it should "upload Invalid plugin zip file url and Throw Client Exception" in {
        ...
        val exception = intercept[ClientException] {
            new PluginMimeTypeMgrImpl().upload("org.ekstep.video", new Node(), "https://ekstep-public-dev.s3-ap-south-1.amazonaws.com/content/do_11218758555843788817/artifact/akshara_kan_1487743191313.zip", null)
        }
    ...
    it should "upload Invalid plugin zip file and Throw Client Exception" in {
        implicit val ss = new StorageService
        val exception = intercept[ClientException] {
            new PluginMimeTypeMgrImpl().upload("org.ekstep.video", new Node(), new File(Resources.getResource("validEcmlContent.zip").toURI), null)
        }
        exception.getMessage shouldEqual "Error !Invalid Content Package File Structure. | [manifest.json should be at root location]"
    }
    ...
    it should "upload Invalid File for plugin and Throw Client Exception" in {
        implicit val ss = new StorageService
        val exception = intercept[ClientException] {
            new PluginMimeTypeMgrImpl().upload("org.ekstep.video", new Node(), new File(Resources.getResource("sample.pdf").toURI), null)
        }
