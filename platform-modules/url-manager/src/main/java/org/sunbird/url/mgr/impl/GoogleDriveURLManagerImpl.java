	...
	public Map<String, Object> validateURL(String url, String validationCriteria) {
		if (StringUtils.isNotBlank(validationCriteria) && validCriteria.contains(validationCriteria)) {
			...
			Map<String, Object> result = new HashMap<String, Object>() {{
				put("value", size);
				put("valid", size <= sizeLimit);
			}};
			return result;
		} else
			throw new ClientException(ResponseCode.CLIENT_ERROR.name(), "Please Provide Valid Criteria For Validation. Supported Criteria : " + validCriteria);
	}
