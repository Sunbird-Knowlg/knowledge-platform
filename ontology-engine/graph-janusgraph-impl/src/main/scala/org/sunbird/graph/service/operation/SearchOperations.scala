...
object SearchOperations {
  ...
  def getNodeByUniqueId(graphId: String, nodeId: String, getTags: Boolean, request: Request, propTypeMap: Option[Map[String, AnyRef]]): Future[Node] = {
      ...
      try {
        ...
        val cs: CompletableFuture[Node] = client.submitAsync(SearchUtil.getVertexByUniqueIdQuery(parameterMap)).toCompletableFuture
          .thenCompose[Node](resultSet => resultSet.all().thenApply[Node](results => {
            val resultList = prepareResultList(results)
            var node: Node = null
            ...
            val nodeMap = new util.HashMap[AnyRef, AnyRef]
            ...
            resultList.forEach { result =>
              if (null != result)
                getRecordValues(result, nodeMap, relationMap, startNodeMap, endNodeMap)
            }

            if (!nodeMap.isEmpty) {
              val entry = nodeMap.entrySet().iterator().next()
              node = GremlinVertexUtil.getNode(graphId, entry.getValue.asInstanceOf[Vertex], relationMap, startNodeMap, endNodeMap, propTypeMap)
            }
            node
  ...
  def getNodeProperty(graphId: String, nodeId: String, key: String): Future[Property] = {
      if (StringUtils.isBlank(graphId))
        throw new ClientException(DACErrorCodeConstants.INVALID_GRAPH.name,
      ...
      try {
        ...
        val cs: CompletableFuture[Property] = client.submitAsync(SearchUtil.getNodePropertyQuery(parameterMap)).toCompletableFuture
          .thenCompose[Property](resultSet => resultSet.all().thenApply[Property](resultSet => {

            val elementMap = resultSet.get(0).getObject.asInstanceOf[util.Map[AnyRef, AnyRef]]
        ...
        case e: Throwable =>
          ...
          e.getCause match {
            case _: NoSuchElementException | _: ResourceNotFoundException =>
              throw new ResourceNotFoundException(DACErrorCodeConstants.NOT_FOUND.name, DACErrorMessageConstants.NODE_NOT_FOUND + " | [Invalid Node Id.]: " + nodeId, nodeId)
            case _ =>
  ...
  def checkCyclicLoop(graphId: String, startNodeId: String, relationType: String, endNodeId: String): util.Map[String, AnyRef] = {
    ...
    val client = ClientUtil.getGraphClient(graphId, GraphOperation.READ)
    val result = client.submit(SearchUtil.generateCheckCyclicLoopQuery(parameterMap)).one()
    if (null != result ) {
  ...
  private def getRecordValues(result: util.Map[String, AnyRef], nodeMap :util.Map[AnyRef, AnyRef], relationMap :util.Map[AnyRef, AnyRef], startNodeMap :util.Map[AnyRef, AnyRef], endNodeMap :util.Map[AnyRef, AnyRef] ): Unit = {
    if (null != nodeMap) {
        ...
        if (null != vertexValue && vertexValue.isInstanceOf[Vertex]) {
            val gremlinVertex : Vertex = result.get(CypherQueryConfigurationConstants.DEFAULT_CYPHER_NODE_OBJECT).asInstanceOf[Vertex]
    ...
    if (null != relationMap) {
        ...
        if (null != edgeValue && edgeValue.isInstanceOf[Edge]) {
            val edge: Edge = result.get(CypherQueryConfigurationConstants.DEFAULT_CYPHER_RELATION_OBJECT).asInstanceOf[Edge]
    ...
    if (null != startNodeMap) {
        ...
        if (null != startVertexValue && startVertexValue.isInstanceOf[Vertex]) {
            val startVertex: Vertex = result.get(CypherQueryConfigurationConstants.DEFAULT_CYPHER_START_NODE_OBJECT).asInstanceOf[Vertex]
    ...
    if (null != endNodeMap) {
        ...
        if (null != endVertexValue && endVertexValue.isInstanceOf[Vertex]) {
            val endVertex: Vertex = result.get(CypherQueryConfigurationConstants.DEFAULT_CYPHER_END_NODE_OBJECT).asInstanceOf[Vertex]
  ...
  def getNodeByUniqueIds(graphId: String, searchCriteria: SearchCriteria): Future[util.ArrayList[Node]]  = {
    ...
    try {
      ...
      val cs: CompletableFuture[util.ArrayList[Node]] = client.submitAsync(SearchUtil.generateGetNodeByUniqueIdsQuery(graphId, searchCriteria)).toCompletableFuture
        .thenCompose[util.ArrayList[Node]](resultSet => resultSet.all().thenApply[util.ArrayList[Node]](results => {
          ...
          if (!CollectionUtils.isEmpty(results)) {
            for (result <- results) {
              ...
              resMap.put(CypherQueryConfigurationConstants.DEFAULT_CYPHER_NODE_OBJECT, res.get(CypherQueryConfigurationConstants.DEFAULT_CYPHER_NODE_OBJECT).asInstanceOf[Vertex])
            ...
            if (!CollectionUtils.isEmpty(finalList)) {
              ...
              if (!vertexMap.isEmpty) {
                for (entry <- vertexMap.entrySet) {
                  nodes.add(GremlinVertexUtil.getNode(graphId, entry.getValue.asInstanceOf[Vertex], relationMap, startNodeMap, endNodeMap))
                }
              }
            }
          }
  ...
  def prepareResultList(results: util.List[Result]): util.List[util.Map[String, AnyRef]] = {
    val finalList = new util.ArrayList[util.Map[String, AnyRef]]
    if (null != results) {
      for (result <- results) {
        val res = result.getObject.asInstanceOf[util.Map[String, AnyRef]]
