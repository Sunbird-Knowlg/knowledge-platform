...
public class GremlinQueryBuilder {
     ...
     */
    public static GraphTraversal<Vertex, Vertex> getVertexByIdentifier(GraphTraversalSource g, String graphId,
            String identifier) {
        return g.V()
     ...
     */
    public static GraphTraversal<Vertex, Vertex> createVertex(GraphTraversalSource g, String graphId,
     ...
     */
    public static GraphTraversal<Vertex, Vertex> updateVertex(GraphTraversalSource g, String graphId, String identifier,
            Map<String, Object> properties) {
     ...
     * Delete vertex
     */
    public static GraphTraversal<Vertex, Vertex> deleteVertex(GraphTraversalSource g, String graphId,
            String identifier) {
     ...
     * This implements Neo4j MERGE functionality - creates edge only if it doesn't
     * exist
     ...
     */
    public static GraphTraversal<Vertex, Edge> getOutgoingEdges(GraphTraversalSource g, String graphId,
            String identifier, String relationType) {
     ...
     */
    public static GraphTraversal<Vertex, Edge> getIncomingEdges(GraphTraversalSource g, String graphId,
            String identifier, String relationType) {
        GraphTraversal<Vertex, Vertex> traversal = getVertexByIdentifier(g, graphId, identifier);
     ...
     */
    public static GraphTraversal<Vertex, Vertex> searchVerticesByProperty(GraphTraversalSource g, String graphId,
     ...
     * This implements Neo4j MERGE behavior with ON CREATE SET and ON MATCH SET
     * clauses
     * 
    ...
    /**
     * Check for cyclic loops in the graph
     * 
     * @param g            GraphTraversalSource
     * @param graphId      Graph identifier
     * @param startNodeId  Start node ID
     * @param relationType Relationship type to traverse
     * @param endNodeId    End node ID (potential cycle closer)
     * @return true if a cycle is detected
     */
    public static boolean checkCyclicLoop(GraphTraversalSource g, String graphId, String startNodeId,
            String relationType, String endNodeId) {
        try {
            // MATCH (start)-[:type*1..]->(end) return end
            return g.V()
                    .has(SystemProperties.IL_UNIQUE_ID.name(), startNodeId)
                    .has("graphId", graphId)
                    .repeat(__.out(relationType))
                    .until(__.has(SystemProperties.IL_UNIQUE_ID.name(), endNodeId).or().loops().is(P.gt(10))) // Safely
                                                                                                              // limit
                                                                                                              // depth
                                                                                                              // to
                                                                                                              // avoid
            ...
            TelemetryManager.error("Error checking cyclic loop", e);
            return false;
        }
    }

    /**
     * Get nodes matching complex search criteria
     * 
     * @param g  GraphTraversalSource
     * @param sc SearchCriteria
     * @return GraphTraversal of matching vertices
     */
    public static GraphTraversal<Vertex, Vertex> getNodesBySearchCriteria(GraphTraversalSource g, SearchCriteria sc) {
        GraphTraversal<Vertex, Vertex> traversal = g.V();

        // 1. Metadata Criteria (includes Identifiers - apply FIRST for index usage)
        List<MetadataCriterion> metadata = sc.getMetadata();
        if (metadata != null && !metadata.isEmpty()) {
            for (MetadataCriterion mc : metadata) {
                applyMetadataCriterion(traversal, mc);
            }
        }

        // 2. GraphId (Partitioning)
        traversal.has("graphId", sc.getGraphId());

        // 3. System Properties (NodeType, ObjectType)
        if (StringUtils.isNotBlank(sc.getNodeType())) {
            traversal = traversal.has(SystemProperties.IL_SYS_NODE_TYPE.name(), sc.getNodeType());
        }
        if (StringUtils.isNotBlank(sc.getObjectType())) {
            traversal = traversal.has(SystemProperties.IL_FUNC_OBJECT_TYPE.name(), sc.getObjectType());
        }

        // 4. Relation Criteria
        List<RelationCriterion> relations = sc.getRelations();
        if (relations != null && !relations.isEmpty()) {
            for (RelationCriterion rc : relations) {
                applyRelationCriterion(traversal, rc);
            }
        }

        // 4. Sorting
        List<Sort> sortOrder = sc.getSortOrder();
        if (sortOrder != null && !sortOrder.isEmpty()) {
            for (Sort sort : sortOrder) {
                if (StringUtils.equalsIgnoreCase(Sort.SORT_DESC, sort.getSortOrder())) {
                    traversal = traversal.order().by(sort.getSortField(),
                            org.apache.tinkerpop.gremlin.process.traversal.Order.desc);
                } else {
                    traversal = traversal.order().by(sort.getSortField(),
                            org.apache.tinkerpop.gremlin.process.traversal.Order.asc);
                }
            }
        }
        ...
        if (sc.getResultSize() > 0) {
            traversal = traversal.range(sc.getStartPosition(), sc.getStartPosition() + sc.getResultSize());
        } else if (sc.getStartPosition() > 0) {
            // If ONLY start position is given (skip), but no limit?
            // Gremlin range is [start, end), so to skip N we range(N, -1) but -1 isn't
            // infinite in standard API usually
            // Using range(start, Long.MAX_VALUE)
            traversal = traversal.range(sc.getStartPosition(), Long.MAX_VALUE);
        }

        return traversal;
    }

    private static void applyMetadataCriterion(GraphTraversal<?, ?> traversal, MetadataCriterion mc) {
        List<Filter> filters = mc.getFilters();
        List<MetadataCriterion> nestedMetadata = mc.getMetadata();
        String op = mc.getOp();

        if (StringUtils.equalsIgnoreCase(SearchConditions.LOGICAL_OR, op)) {
            List<GraphTraversal<?, ?>> criteriaList = new ArrayList<>();

            // Collect all filters as traversals
            if (filters != null) {
                for (Filter filter : filters) {
                    GraphTraversal<?, ?> filterTraversal = __.start();
                    applyFilter(filterTraversal, filter);
                    criteriaList.add(filterTraversal);
                }
            }

            // Collect nested metadata as traversals
            if (nestedMetadata != null) {
                for (MetadataCriterion nestedMc : nestedMetadata) {
                    GraphTraversal<?, ?> nestedTraversal = __.start();
                    applyMetadataCriterion(nestedTraversal, nestedMc);
                    criteriaList.add(nestedTraversal);
                }
            }

            if (!criteriaList.isEmpty()) {
                GraphTraversal<?, ?>[] criteriaArray = criteriaList.toArray(new GraphTraversal[0]);
                traversal.or(criteriaArray);
            }
        } else {
            // Default AND - Apply directly to main traversal for better optimization
            if (filters != null) {
                for (Filter filter : filters) {
                    applyFilter(traversal, filter);
                }
            }

            if (nestedMetadata != null) {
                for (MetadataCriterion nestedMc : nestedMetadata) {
                    applyMetadataCriterion(traversal, nestedMc);
                }
            }
        }
    }

    private static void applyFilter(GraphTraversal<?, ?> traversal, Filter filter) {
        ...
        switch (operator) {
            ...
            case SearchConditions.OP_NOT_EQUAL:
                traversal.has(property, P.neq(value));
                break;
            case SearchConditions.OP_GREATER_THAN:
                traversal.has(property, P.gt(value));
                break;
            case SearchConditions.OP_GREATER_OR_EQUAL:
                traversal.has(property, P.gte(value));
                break;
            case SearchConditions.OP_LESS_THAN:
                traversal.has(property, P.lt(value));
                break;
            case SearchConditions.OP_LESS_OR_EQUAL:
                traversal.has(property, P.lte(value));
                break;
            case SearchConditions.OP_IN:
                if (value instanceof List) {
                    traversal.has(property, P.within((List<?>) value));
                } else if (value instanceof Object[]) {
                    traversal.has(property, P.within((Object[]) value));
                } else {
                    traversal.has(property, value);
                }
                break;
            case SearchConditions.OP_LIKE:
                // Case-insensitive check is hard in standard Gremlin without backend-specifics,
                // but JanusGraph supports TextP.textContains or similar for ES
                // Using standard Gremlin TextP for now
                traversal.has(property, TextP.containing(String.valueOf(value)));
                break;
            case SearchConditions.OP_STARTS_WITH:
                traversal.has(property, TextP.startingWith(String.valueOf(value)));
                break;
            case SearchConditions.OP_ENDS_WITH:
                traversal.has(property, TextP.endingWith(String.valueOf(value)));
                break;
            default:
                traversal.has(property, value);
                break;
    ...
    private static void applyRelationCriterion(GraphTraversal<?, ?> traversal, RelationCriterion rc) {
        String relationName = rc.getName();
        String direction = rc.getDirection() != null ? rc.getDirection().name()
                : RelationCriterion.DIRECTION.OUT.name();
        // handling simple relations for now, deep traversal with hierarchy needs
        // recursive handling similar to above

        GraphTraversal<?, ?> relTraversal;
        if (StringUtils.equalsIgnoreCase(RelationCriterion.DIRECTION.IN.name(), direction)) {
            relTraversal = __.inE(relationName).outV();
        } else if (StringUtils.equalsIgnoreCase(RelationCriterion.DIRECTION.BOTH.name(), direction)) {
            relTraversal = __.bothE(relationName).otherV();
        } else {
            relTraversal = __.outE(relationName).inV();
        }

        // Apply filters on the related node if any
        List<String> identifiers = rc.getIdentifiers();
        if (identifiers != null && !identifiers.isEmpty()) {
            relTraversal.has(SystemProperties.IL_UNIQUE_ID.name(), P.within(identifiers));
        }

        String objectType = rc.getObjectType();
        if (StringUtils.isNotBlank(objectType)) {
            relTraversal.has(SystemProperties.IL_FUNC_OBJECT_TYPE.name(), objectType);
        }

        // Apply nested metadata on related node
        List<MetadataCriterion> metadata = rc.getMetadata();
        if (metadata != null && !metadata.isEmpty()) {
            for (MetadataCriterion mc : metadata) {
                applyMetadataCriterion(relTraversal, mc);
            }
        }

        if (rc.isOptional()) {
            // Optional match logic ... where(optional(traversal))
            // Gremlin doesn't have direct "optional match" in one step like Neo4j,
            // but we can use optional() step
            // traversal.optional(relTraversal)
            // However, optional() moves the traverser.
            // Neo4j's OPTIONAL MATCH (ee)-[]-() means "if it exists, return it, else null"
            // typically used for "filtering" (WHERE) vs "retrieving".
            // If it's used as a filter (must exist), use where(). If optional, use
            // optional()??

            // The Neo4j query builder appends it to MATCH clause.
            // If it has filters, it acts as a filter.
            // For now, assuming standard AND behavior unless optional
            // But implementing strict optional match inside a filter context is complex.
            // If this RelationCriterion is a FILTER, it implies existence check.
            // If it is optional, we might skip enforcement?
            // Since we are inside a "getNodes" flow, we likely want to filter main nodes
            // that HAVE this relation.

            if (!rc.isOptional()) {
                traversal.where(relTraversal);
            }
        } else {
            traversal.where(relTraversal);
        }
