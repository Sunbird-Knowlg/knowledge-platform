    ...
    public void testRelationModel_2(){
        ...
        try (Session session = driver.session()) {
            ...
            StatementResult result2 = session
                    .run("MATCH (n:domain{IL_UNIQUE_ID:'do_1129067102240194561252'}) match (m:domain{IL_UNIQUE_ID:'do_11232724509261824014'}) CREATE (m)-[r:associatedTo]->(n) return m,n,r");
            for (Record record : result2.list()) {
                org.neo4j.driver.v1.types.Node startNode = record.get("m").asNode();
                org.neo4j.driver.v1.types.Node endNode = record.get("n").asNode();
                ...
                org.neo4j.driver.v1.types.Relationship relationships = record.get("r").asRelationship();
                Relation relation_6 = new Relation(graphId, relationships, startNodeMap, endNodeMap);
            }
        }
    }
