...
abstract class IDefinition(graphId: String, schemaName: String, version: String = "1.0", ocd: ObjectCategoryDefinition = ObjectCategoryDefinition())(implicit ec: ExecutionContext, oec: OntologyEngineContext) {//extends CoreDomainObject(graphId, schemaName, version, categoryId) {
    ...
    @throws[Exception]
    def getNode(identifier: String, operation: String = "read", mode: String, versioning: Option[String] = None, disableCache: Option[Boolean] = None , propTypeMap: Option[Map[String, AnyRef]] = None)(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[Node]
