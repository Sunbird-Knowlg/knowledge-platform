...
class TestDataNode extends BaseSpec {
    ...
    "update live node with external props" should "update image node with existing external props in image node" ignore {
        ...
        updateFuture.map(node => {
            ...
            val result: StatementResult = graphDb.run("Match (n:domain{IL_UNIQUE_ID:'do_1129067102240194561252.img'}) return n.status as status, n.prevStatus as prevStatus")
    ...
    "update live collection with external props" should "update image node with existing external props in image node for collection" ignore {
        ...
        updateFuture.map(node => {
            ...
            val result: StatementResult = graphDb.run("Match (n:domain{IL_UNIQUE_ID:'do_11283193441064550414.img'}) return n.status as status, n.prevStatus as prevStatus")
            val resMap = result.next()
            assert("Draft".contentEquals(resMap.get("status").asInstanceOf[String]))
            assert("Live".contentEquals(resMap.get("prevStatus").asInstanceOf[String]))
        })
    ...
    "read Live node twice one from neo4j and one from cache" should "read node from neo4j and from cache" in {
        ...
        createRelationData()
        graphDb.run("MATCH (n:domain{IL_UNIQUE_ID:'do_1129067102240194561252'}) match (m:domain{IL_UNIQUE_ID:'Num:C3:SC2'}) CREATE (n)-[r:associatedTo]->(m)")
        graphDb.run("MATCH (n:domain{IL_UNIQUE_ID:'do_1129067102240194561252'}) match (m:domain{IL_UNIQUE_ID:'do_11232724509261824014'}) CREATE (m)-[r:associatedTo]->(n)")
        val request = new Request()
        request.setObjectType("Content")
        request.setContext(getContextMap())
    ...
    "bulkUpdate with single node" should "should update the node successfully" ignore {
