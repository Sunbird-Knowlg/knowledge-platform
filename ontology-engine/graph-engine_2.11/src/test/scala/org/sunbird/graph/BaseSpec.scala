...
class BaseSpec extends AsyncFlatSpec with Matchers with BeforeAndAfterAll {
    ...
    private val script_2 = "CREATE TABLE IF NOT EXISTS content_store.content_data (identifier text, last_updated_on timestamp,body blob,oldBody blob,stageIcons blob,PRIMARY KEY (identifier));"
    ...
    def setUpEmbeddedNeo4j(): Unit = {
        if(null == graphDb) {
            ...
            graphDb = new GraphDatabaseFactory()
                            .newEmbeddedDatabaseBuilder(new File("/data/testingGraphDB"))
