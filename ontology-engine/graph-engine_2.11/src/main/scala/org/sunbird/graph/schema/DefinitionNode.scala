...
object DefinitionNode {
    ...
    def validate(request: Request, setDefaultValue: Boolean = true)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[Node] = {
      ...
      val objectCategoryDefinition: ObjectCategoryDefinition = getObjectCategoryDefinition(request.getRequest.get("primaryCategory").asInstanceOf[String], schemaName, request.getContext.getOrDefault("channel", "all").asInstanceOf[String])
    ...
    def getObjectCategoryDefinition(primaryCategory: String, schemaName: String, channel: String = "all"): ObjectCategoryDefinition = {
      if(StringUtils.isNotBlank(primaryCategory))
        ObjectCategoryDefinition(primaryCategory, schemaName, channel)
