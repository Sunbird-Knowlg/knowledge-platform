...
object AccessVerifierUtil {
  ...
  def checkAccess(accessRules: List[AnyRef], request: Request): Boolean = {
    ...
    if(StringUtils.isEmpty(accessToken)) throw new ClientException("ERR_CONTENT_ACCESS_RESTRICTED", "You are not permitted to view this content!")
    ...
    try {
      ...
      case e: ClientException => throw new ClientException("ERR_CONTENT_ACCESS_RESTRICTED", "You are not permitted to view this content!")
    }
  }
