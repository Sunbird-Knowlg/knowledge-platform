        ...
        if (StringUtils.isNotBlank(artifactUrl) && !restrictedMimeTypesForUpload.contains(mimeType)) {
            ...
            val uploadFuture = if (isInternalUrl(artifactUrl)) {
                ...
                if (mimeTypeManager.isInstanceOf[H5PMimeTypeMgrImpl])
                    ...
                    mimeTypeManager.upload(copiedNode.getIdentifier, copiedNode, file, null)
            } else mimeTypeManager.upload(copiedNode.getIdentifier, copiedNode, node.getMetadata.getOrDefault(CopyConstants.ARTIFACT_URL, "").asInstanceOf[String], null)
