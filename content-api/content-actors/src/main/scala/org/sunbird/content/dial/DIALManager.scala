...
object DIALManager {
	...
	val CLOUD_STORAGE_ENDPOINT: String = Platform.getString("cloud_storage_endpoint", "https://dev.knowlg.sunbird.org")
  ...
  * */
	...
	def createRequest(data: Map[String, AnyRef], channel: String, publisher: Option[String], rspObj: Response, request: Request)(implicit oec: OntologyEngineContext, ec: ExecutionContext) = {
		...
		val dialCodesMap = data.map { case (dialcode, index) =>
			...
			val dialData = Map(
				"data" -> s"$CLOUD_STORAGE_ENDPOINT/$DIAL_CONTAINER/$dialcode",
				"text" -> dialcode,
				"id" -> fileName
			)
