    ...
    def getLatestLeafNodes(leafNodeIds : util.List[String])(implicit oec: OntologyEngineContext, ec: ExecutionContext) = {
        if(CollectionUtils.isNotEmpty(leafNodeIds)) {
            ...
            DataNode.list(request).map(nodes => {
                val leafNodeMap: Map[String, AnyRef] = nodes.toList.map(node => (node.getIdentifier, NodeUtil.serialize(node, null, node.getObjectType.toLowerCase, HierarchyConstants.SCHEMA_VERSION, true).asInstanceOf[AnyRef])).toMap
                ...
                DataNode.list(request).map(imageNodes => {
                    ...
                    val imageLeafNodeMap: Map[String, AnyRef] = imageNodes.toList.map(imageNode => {
                        ...
                        val metadata = NodeUtil.serialize(imageNode, null, imageNode.getObjectType.toLowerCase, HierarchyConstants.SCHEMA_VERSION, true)
