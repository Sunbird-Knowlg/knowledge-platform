...
import taxonomy.utils.{Constants, JavaJsonUtils}
...
abstract class BaseController(protected val cc: ControllerComponents)(implicit exec: ExecutionContext) extends AbstractController(cc) {
    ...
    def setRequestContext(request:org.sunbird.common.dto.Request, version: String, objectType: String, schemaName: String): Unit = {
        val existingContext = if (request.getContext != null) request.getContext else new java.util.HashMap[String, AnyRef]()
        existingContext.put("graph_id", "domain")
        existingContext.put("version", version)
        existingContext.put("objectType", objectType)
        existingContext.put("schemaName", schemaName)
        request.setObjectType(objectType);
        request.setContext(existingContext)
    }
}
