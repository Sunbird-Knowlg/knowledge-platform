...
class SearchSpec extends BaseSpec {
    ...
    "SearchApp" should {
        ...
        "return client error response for search API if visibility is 'Private'" in {
            val controller = app.injector.instanceOf[controllers.SearchController]
        ...
        "return success response for private search API" in {
            val controller = app.injector.instanceOf[controllers.SearchController]
        ...
        "return client error response for private search API if 'channel' is invalid" in {
            val controller = app.injector.instanceOf[controllers.SearchController]
            val json: JsValue = Json.parse("""{"CHANNEL_ID":"","request": {"filters": {"visibility": ["Private"]}}}""")
            val fakeRequest = FakeRequest("POST", "/v3/private/search").withJsonBody(json).withHeaders(FakeHeaders())
            val result = controller.search()(fakeRequest)
            status(result) must equalTo(BAD_REQUEST)
        }
