...
/**
 * Restored SearchProcessorTest. The original test used asynchronous onSuccess/onComplete
 * handlers; to simplify migration and keep tests deterministic we synchronously Await
 * the scala.concurrent.Future results and assert on returned maps. This keeps changes
 * confined to test sources only and avoids Pekko onSuccess->onComplete migration issues.
 */
public class SearchProcessorTest extends BaseSearchTest {
    ...
    private static final Duration AWAIT_TIMEOUT = Duration.create(10, TimeUnit.SECONDS);
    ...
    public void testSearchByQuery() throws Exception {
        ...
        Map<String, Object> success = Await.result(res, AWAIT_TIMEOUT);
    ...
    public void testSearchFacets() throws Exception {
        ...
        Map<String, Object> success = Await.result(res, AWAIT_TIMEOUT);
        Assert.assertNotNull(success);
        List<Object> list = (List<Object>) success.get("facets");
