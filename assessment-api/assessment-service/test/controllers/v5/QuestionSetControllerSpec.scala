...
class QuestionSetControllerSpec extends BaseSpec {
  ...
  "reject should update the questionSet status to Draft successfully for given valid identifier" in {
  ...
  "copy should create a cloned questionSet successfully for given valid request" in {
    val result = controller.copy("do_123", None, "deep")(FakeRequest())
    isOK(result)
    status(result)(defaultAwaitTimeout) must equalTo(OK)
  }
