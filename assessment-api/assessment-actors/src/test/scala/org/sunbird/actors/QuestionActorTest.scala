...
class QuestionActorTest extends BaseSpec with MockFactory with copyTrait {
	...
	it should "return success response for 'copyQuestion'" in {
		...
		val nodes: util.List[Node] = getCategoryNode()
		(graphDB.getNodeByUniqueIds(_: String, _: SearchCriteria)).expects(*, *).returns(Future(nodes)).anyNumberOfTimes()
		(graphDB.getNodeByUniqueId(_: String, _: String, _: Boolean, _: Request)).expects("domain", "do_1234", false, *).returns(Future(getExistingQuestionNode())).anyNumberOfTimes()
		...
		(graphDB.readExternalProps(_: Request, _: List[String])).expects(*, *).returns(Future(getReadPropsResponseForQuestion())).anyNumberOfTimes()
		(graphDB.addNode(_: String, _: Node)).expects(*, *).returns(Future(getNewQuestionNode()))
