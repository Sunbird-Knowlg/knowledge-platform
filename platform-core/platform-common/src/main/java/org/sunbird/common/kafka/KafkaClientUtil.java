package org.sunbird.common.kafka;
...
public class KafkaClientUtil {
    ...
    public static void send(String event, String topic) throws Exception {
        boolean isTopicCheckReq = !Platform.config.hasPath("kafka.topic.send.enable") || Platform.config.getBoolean("kafka.topic.send.enable");
        if (!isTopicCheckReq)
            return;
        if (validate(topic)) {
